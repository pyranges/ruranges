...
def overlaps(
    *,
    ...
    length = check_lengths(starts, ends, groups)
    length2 = check_lengths(starts2, ends2, groups2)

    groups_validated, groups2_validated = validate_two_groups(length, length2, groups, groups2)
...
def merge(
    *,
    ...
    from _ruranges import merge_numpy  # type: ignore[attr-defined]
    ...
    idx, _starts, _ends, counts = merge_numpy(
        groups_validated,
        starts.astype(np.int64),
        ends.astype(np.int64),
        slack=slack,
...
def check_lengths(a: NDArray, b: NDArray, c: NDArray | None) -> int:
    """
    Checks that the three provided arrays all have the same length.

    Args:
        a, b, c: Arrays to be compared.

    Returns:
        The common length of the arrays.

    Raises:
        ValueError: If any of the arrays has a different length.
    """
    n = len(a)
...
def check_array_lengths(
    ...
    """
    Checks that the following conditions hold:
    
    - The first group (consisting of `starts`, `ends`, and, if provided, `groups`)
      all have the same length.
    - The second group (consisting of `starts2`, `ends2`, and, if provided, `groups2`)
      all have the same length.
    
    Returns:
        A tuple (n, n2) where:
            n  is the common length of starts, ends, and groups (if provided),
            n2 is the common length of starts2, ends2, and groups2 (if provided).
    
    Raises:
        ValueError: If the length checks for either group fail.
    """
    n = check_lengths(starts, ends, groups)
    n2 = check_lengths(starts2, ends2, groups2)
    return n, n2
...
def validate_group(
    ...
    """
    Validates a single group array.

    If `groups` is provided, it is cast to np.uint32.
    Otherwise, a new array of zeros with the given length is created.

    Args:
        length: The expected length of the group array.
        groups: An optional array of group IDs.

    Returns:
        A numpy array of type np.uint32 with the validated group IDs.
    """
    if groups is None:
        return np.zeros(length, dtype=np.uint32)
    return groups.astype(np.uint32)
...
def validate_two_groups(
    ...
    if groups is None:
        return np.zeros(length, dtype=np.uint32), np.zeros(length2, dtype=np.uint32)
